          ...
          <template v-for="user in userList" :key="user.username">
            <div class="card mb-1">
              ...
              <div class="card-body">
                ...
                <select v-model="user.register_id" @change="changeRegisterId(user)" class="form-select">
                  <option v-for="register in registereList" :key="register.id" :value="register.id">{{ register.name }}</option>
                </select>
              </div>
            </div>
          </template>
...
<script setup>
  ...
  const registereList = reactive([]);
  const dialogStore = useDialogStore();

  const isMobileView = ref(window.innerWidth <= 768);

  async function changeRegisterId(user) {
    console.log('changeRegisterId user', user);
    try {
      const response = await fetch('/api/setUserRegisterId', {
        method: 'POST',
        headers: {
      ...
      if (!response.ok) {
        console.log('Es gab ein Problem mit dem setzen der Register Id');
        setTimeout(() => {
          dialogStore.setParameter('Eehler beim setzen der Register Id', `${response.message}`, 'ok', null, '', null, null);
        }, 1000);
      }
    } catch (error) {
      console.error('Es gab ein Problem mit dem setzen der Register Id', error);
      setTimeout(() => {
        dialogStore.setParameter('Es gab ein Problem mit dem setzen der Register Id', `${error}`, 'ok', null, '', null, null);
      }, 1000);
    }
  }
  ...
  async function getRegisterList() {
    try {
      let response;
      response = await fetch(`/api/getRegisterList`, {
        method: 'GET',
        credentials: 'include'  // Cookies mitsenden
      });
      ...
      if (response.ok) {
        registereList.splice(0);

        for (const registere of result.registereList) {
          registereList.push(registere);
        }
      ...
      console.error('Es gab ein Problem mit dem Abrufen der getRegisterList:', error);
      setTimeout(() => {
